...
from kraken_msgs.msg import thrusterData8Thruster
...
class SetYaw(object):
    """ Creates messages that are used to publish feedback/result to client
	  ...
	  *  pub_thrusters8 : publish as 8 thruster model on thruster converter 
    ...
    thruster8Data=thrusterData8Thruster()
    ...
    def __init__(self, name):
   	...
   	self.pub_thrusters8 = rospy.Publisher(topicHeader.CONTROL_PID_THRUSTER8, thrusterData8Thruster, queue_size = 2)
	...
	#global out
	    ...
	    self.curtime = time.time()
	     ...
	     self.ci += YAW.error * dt
             if dt> 0 :
                 self.cd = YAW.delta_error / dt

             self.pretime = self.curtime
            

             out = self.P * YAW.error + self.I * self.ci + self.D * self.cd
             if out > self.max:
                 out = self.max
             elif out < self.min:
                 out = self.min
	    ...
	    print "Yaw.error : ",YAW.error,"prevError : ",prevError, "Delta_error : ",YAW.delta_error, "PID Output : ",out
	    ...
	    # 8 Thruster model
            self.thruster8Data.data[0] = 0.0
            self.thruster8Data.data[1] = 0.0
            self.thruster8Data.data[2] = 0.0 #ControlOutput  # Left Thruster
            self.thruster8Data.data[3] = 0.0 #-1 * ControlOutput # Rigt Thruster
            self.thruster8Data.data[4] = 0.0     
            self.thruster8Data.data[5] = 0.0
	    self.thruster8Data.data[6] = 0.0
	    self.thruster8Data.data[7] = 0.0     
            self.thruster8Data.header  = std_msgs.msg.Header()
            self.thruster8Data.header.stamp = rospy.Time.now()
